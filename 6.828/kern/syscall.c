...
sys_exofork(void)
{
	...
	if ((error_code = env_alloc(&env, curenv->env_id)) < 0) {
		return error_code;
	}
...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	uint32_t error_code;
	struct Env* env = NULL;
	if ((error_code = envid2env(envid, &env, true)) < 0) {
		return error_code;
	}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if ((srcpage = page_lookup(srcenv->env_pgdir, srcva, &srcpte)) == NULL) {
		return -E_INVAL;
	}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if ((error_code = envid2env(envid, &dstenv, 0)) < 0) {
		return error_code;
	}
	...
	if ((uintptr_t)srcva < UTOP && (uintptr_t)(dstenv->env_ipc_dstva) < UTOP) {
		...
		pte_t* p_srcpte;
		struct PageInfo* srcpg = page_lookup(curenv->env_pgdir, srcva, &p_srcpte); 

		if (!(*p_srcpte & PTE_P)) {
			return -E_INVAL;
		}

		if (!((*p_srcpte) & PTE_W) && (perm & PTE_W)) {
			return -E_INVAL;
		}

		if((error_code = page_insert(dstenv->env_pgdir, srcpg, dstenv->env_ipc_dstva, perm)) < 0)
			return error_code;
