...
page_init(void)
{
	...
	for (i = 1; i < npages_basemem; i++) {
		if (&pages[i] == pa2page(MPENTRY_PADDR)) continue;
		pages[i].pp_ref = 0;
...
{
	// Fill this function in
	pde_t* p_fPte = pgdir + PDX(va);
	pte_t* p_sPte = NULL;

	if (*p_fPte != 0 && *p_fPte & PTE_P) {
		p_sPte = (pte_t*)KADDR(PTE_ADDR(*p_fPte));
	} else if (create) {
		...
		new_page->pp_ref++;
		*p_fPte = (page2pa(new_page) | PTE_P | PTE_U | PTE_W);
		p_sPte = (pte_t*)KADDR(PTE_ADDR(*p_fPte));
	} else {
		return NULL;
	}
	return p_sPte + PTX(va);
}
...
{
	// Fill this function in
	for (uintptr_t i = 0; i < (uintptr_t)size; i += PGSIZE, va += PGSIZE, pa += PGSIZE) {
		pte_t* page_table = pgdir_walk(pgdir, (void *)va, 1);
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	*page_table = page2pa(pp) | perm | PTE_P;
	return 0;
}
...
{
	// Fill this function in
	pte_t* p_sPte = pgdir_walk(pgdir, va, 0);
	if (p_sPte == NULL) {
		return NULL;
	}

	if (pte_store != NULL)  {
		*pte_store = (pte_t*)p_sPte;
	}

	return pa2page(PTE_ADDR(*p_sPte));
}
...
page_remove(pde_t *pgdir, void *va)
{
	// Fill this function in
	pte_t* p_sPte = NULL;
	struct PageInfo* ppInfo = page_lookup(pgdir, va, &p_sPte);
	if (ppInfo == NULL) {
		return;
	} 
	page_decref(ppInfo);
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	if (base + size > MMIOLIM) {
		panic("in mmio_map_region, base out of range");
	}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for (; addr < end; addr += PGSIZE) {
		...
		if (!(pte = pgdir_walk(env->env_pgdir, (const void*)addr, 0))) {
			iserror = 1;
			break;
		}
...
check_va2pa(pde_t *pgdir, uintptr_t va)
{
	...
	if (!(*pgdir & PTE_P)){
		// cprintf("%ud", *(pgdir));
		// cprintf("pgdir problem\n");
		return ~0;
	}
	p = (pte_t*) KADDR(PTE_ADDR(*pgdir));
	if (!(p[PTX(va)] & PTE_P)){
		// cprintf("pgtbl problem\n");
		return ~0;
	}
...
check_page(void)
{
	...
	// cprintf("%d, %d\n\n\n", check_va2pa(kern_pgdir, 0), page2pa(pp1));
