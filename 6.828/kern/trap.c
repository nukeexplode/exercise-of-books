...
trap_init(void)
{
	...
	SETGATE(idt[T_BRKPT], 0, GD_KT, BRKPT, 3);//ex 6
...
trap_dispatch(struct Trapframe *tf)
{
	...
	if (tf->tf_trapno == T_PGFLT) {
		...
		uint32_t eax = syscall(regs->reg_eax, regs->reg_edx, regs->reg_ecx, regs->reg_ebx, regs->reg_edi, regs->reg_esi);
		regs->reg_eax = eax;
		return;
	}
...
page_fault_handler(struct Trapframe *tf)
{
	...
	// LAB 3: Your code here.
	if ((tf->tf_cs & 3) == 0) {
		panic("page fault in kernel at %d", fault_va);
	}
	...
	if (curenv->env_pgfault_upcall) {
		if (curenv->env_tf.tf_esp < UXSTACKTOP && curenv->env_tf.tf_esp > UXSTACKTOP - PGSIZE) {
			uxtf = (struct UTrapframe*)(tf->tf_esp - sizeof(struct UTrapframe) - 4);
		}else {
			uxtf = (struct UTrapframe*)(UXSTACKTOP - sizeof(struct UTrapframe));
		}
		// cprintf("uxtf:%x\n", (size_t)uxtf);
		user_mem_assert(curenv, uxtf, sizeof(struct UTrapframe), PTE_P | PTE_W);
		...
		curenv->env_tf.tf_esp = (uintptr_t)uxtf;
		env_run(curenv);
	}
