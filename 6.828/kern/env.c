...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	for (uint32_t va_current = va_start; va_current < va_end; va_current += PGSIZE)
	{
		...
		if (page_insert(e->env_pgdir, p, (void *)va_current, PTE_U | PTE_W) < 0)
		{
			panic("in region_alloc: page_insert failed");
		}
	}
}
...
load_icode(struct Env *e, uint8_t *binary)
{
	...
	{
		panic("int load_icode: binary->e_magic != ELF_MAGIC");
	}

	struct Proghdr *ph = (struct Proghdr *)((uint32_t)eh + eh->e_phoff);
	for (uint16_t i = 0; i < eh->e_phnum; i++, ph++)
	{
		if (ph->p_type != ELF_PROG_LOAD)
			continue;
		// cprintf("(1) ph->p_offset : %d\n\n", ph->p_offset);
		region_alloc(e, (void *)ph->p_va, ph->p_memsz);
		memset((void *)ph->p_va, 0, ph->p_memsz);
		// uint32_t va = (uint32_t)binary + ph->p_offset;
		// cprintf("(2) ph->p_offset : %d\n", ph->p_offset);
		memcpy((void *)ph->p_va, (void *)((uint32_t)binary + ph->p_offset), ph->p_filesz);
	}
...
void env_create(uint8_t *binary, enum EnvType type)
{
	...
	struct Env *e = NULL;
	if (env_alloc(&e, 0) < 0)
		panic("in env_create: env_alloc failed");
	load_icode(e, binary);
	if (type == ENV_TYPE_FS)
	{
...
void env_free(struct Env *e)
{
	...
	static_assert(UTOP % PTSIZE == 0); //why????
